  ...
  return (
    ...
    <div>
      <div className='deliver'>
        <b>Deliver to: </b>
        <button onClick={()=>navigate('/chooseadd')} className="deliver_btn">Change</button>
      </div>
      {newResult.length > 0 ? 
      <>
      {newResult.map((ele)=> {
        ...
        <div className='o_add'>
          ...
          <b>{ele.name}</b>  <p style={{color:'grey', marginLeft:'10px'}}>{ele.type}</p>
          </div>
        
          <div>{ele.house} {ele.area} {ele.city} {ele.pincode}</div>
          <p>{ele.phone}</p>
        </div>
        )
      })}
